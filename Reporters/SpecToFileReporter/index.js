...
//  { buildTableData, printTable, getFormattedRows } from './utils'
    ...
    getResultDisplay () {
        ...
        for (const suite of suites) {
            ...
            for (const test of eventsToReport) {
                ...
                output.push(`${testIndent}${(this.getSymbol(state))} ${testTitle}`)

                // print cucumber data table cells
                if (test.argument && test.argument.rows && test.argument.rows.length) {
                    const data = util.buildTableData(test.argument.rows)
                    const rawTable = util.printTable(data)
                    const table = util.getFormattedRows(rawTable, testIndent)
                    output.push(...table)
                }
            }
...
module.exports = SpecToFileReporter
